<template>
  <div
    v-if="(showPipelinesOnly ? (pipelineCount > 0) : true) && showProjectOnlyOn"
...
export default {
  ...
  computed: {
    ...
    config() {
      ...
      if (
        Config.root.projectConfig.hasOwnProperty(
          this.project.path_with_namespace
        )
      ) {
        config = merge(
          config,
          Config.root.projectConfig[this.project.path_with_namespace]
        );
      }
    ...
    pipelines: {
      ...
      handler(pipelines) {
        ...
        const configuredDefaultBranch =
          this.config.default || this.project.default_branch;
        ...
        if (
          pipelines &&
          this.project &&
          this.config.soundAlerts.soundUrl !== null
        ) {
          const pipelinesWithSoundAlertsEnabled = Object.keys(pipelines).filter(
            (branchName) => {
              return (
                !!branchName.match(
                  new RegExp(this.config.soundAlerts.include)
                ) &&
                (!this.config.soundAlerts.exclude ||
                  !branchName.match(
                    new RegExp(this.config.soundAlerts.exclude)
                  ))
              );
            }
          );
          ...
          for (const branch of pipelinesWithSoundAlertsEnabled) {
            const newFailedPipelines = pipelines[branch].filter(
              (p) =>
                p.status === "failed" &&
                !this.alertSoundPlayedForPipelines.includes(p.id)
            );
  ...
  methods: {
    ...
    async fetchPipelines() {
      ...
      const branchNames = branches
        ...
        .filter((branchName) => {
          return (
            !!branchName.match(new RegExp(this.config.include)) &&
            (!this.config.exclude ||
              !branchName.match(new RegExp(this.config.exclude)))
          );
        });
      let tags = [];
      if (showTags) {
        tags = await this.$api(
          `/projects/${this.projectId}/repository/tags`,
          {
            per_page: fetchCount > 100 ? 100 : fetchCount,
          },
          { follow_next_page_links: fetchCount > 100 }
      ...
      let hideSkippedPipelines = Config.getProjectProperty(
        "hideSkippedPipelines",
        this.project.path_with_namespace
      );

      refLoop: for (const refName of refNames) {
        ...
        if (pipelines.length > 0) {
          ...
          for (const pipeline of activePipelines) {
            const resolvedPipeline = await this.$api(
              `/projects/${this.projectId}/pipelines/${pipeline.id}`
            );
            if (
              (maxAge === 0 ||
                (new Date() - new Date(resolvedPipeline.updated_at)) /
                  1000 /
                  60 /
                  60 <=
                  maxAge) &&
              (!hideSkippedPipelines || resolvedPipeline.status !== "skipped")
            ) {
              resolvedPipelines.push(resolvedPipeline);
            }
          }

          const historyCount = Config.getProjectProperty(
            "historyCount",
            this.project.path_with_namespace
          );
          ...
          for (let i = newPipelines[refName].length; i < historyCount; i++) {
            const resolvedPipeline = await this.$api(
              `/projects/${this.projectId}/pipelines/${pipelines[i].id}`
            );
            if (
              (maxAge === 0 ||
                (new Date() - new Date(resolvedPipeline.updated_at)) /
          ...
          if (
            this.config.maxPipelines !== 0 &&
            count >= this.config.maxPipelines
          ) {
            break;
          }
        }
