<template>
  ...
  >
    <div
      :class="['job-circle', job.status === 'failed' ? (job.allow_failure ? 'warning' : 'failed') : job.status, {square: !showJobNames}]"
    >
      ...
      <span v-if="showJobNames" :key="job.name">{{ job.name }}</span>
    </div>
    <div class="pipe"></div>
  </a>
</template>
...
export default {
  ...
  computed: {
    ...
    showJobNames() {
      let showJobName =
        Config.root.showJobs == "name" || Config.root.showJobs == "iconAndName";
    ...
    showJobIcons() {
      return (
        Config.root.showJobs === "icon" ||
        Config.root.showJobs === "iconAndName"
      );
    },
  },
};
