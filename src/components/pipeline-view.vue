  ...
  methods: {
    async fetchJobs() {
      this.jobs = await this.$api(
        `/projects/${this.project.id}/pipelines/${this.pipeline.id}/jobs?per_page=50`
      );
    ...
    excludeRestartedJobs() {
      ...
      this.jobs = this.jobs.filter(
        (job) => jobs_id_by_name[job["name"]] === job["id"]
      );
      this.jobs = this.jobs.sort(
        (j1, j2) =>
          jobs_order.indexOf(j1["name"]) - jobs_order.indexOf(j2["name"])
      );
    },
    setupDurationCounter() {
      const pipeline = this.pipeline;

      const startedAtDiffSeconds =
