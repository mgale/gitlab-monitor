<template>
  <div class="app">
    ...
    <div v-else-if="!configured" class="container">
      <h1>Configuration</h1>
      <p>
        Hi! Before you can use GitLab Monitor, it has to be configured.
        <br />Configuration is done by supplying YAML formatted configuration.
        <br />Your configuration is being persisted in this browser.
      </p>
      <div class="warning">
        <h2>WARNING</h2>There were some breaking changes recently (20191018). Please take
        a look at the
        <a
          href="https://github.com/timoschwarzer/gitlab-monitor/wiki/Update-20191018:-Upgrade-guide"
          target="_blank"
      ...
      <p class="error" v-if="!configIsValid">YAML is invalid!</p>
    ...
    <div v-if="configured" class="configure" @click.prevent.stop="configured = false">Configure</div>
  </div>
</template>
    ...
    sortedProjects() {
      const sortMethods = {
        ...
        nameWithNamespace: (a, b) =>
          a.name_with_namespace.localeCompare(b.name_with_namespace),
      };
  ...
  methods: {
    async fetchProjects() {
      ...
      for (let scope of scopeId) {
        let urlPrefix = "";
        if (
          (scopeType === "users" || scopeType === "groups") &&
          scope !== null
        ) {
        ...
        apiPromises.push(
          this.$api(urlPrefix + "/projects", gitlabApiParams, {
      ...
      this.projects = projects.filter((project) => {
        return (
          ...
          (maxAge === 0 ||
            (new Date() - new Date(project.last_activity_at)) /
              1000 /
              60 /
              60 <=
              maxAge) &&
          // Include rules
          project.path_with_namespace.match(
            new RegExp(Config.root.filter.include)
          ) &&
          // "Exclude untagged" rule
          ((project.tag_list.length > 0 &&
            project.tag_list.some((tag) =>
              tag.match(new RegExp(Config.root.filter.includeTags))
            )) ||
            (project.tag_list.length === 0 &&
          ...
          (Config.root.filter.exclude === null ||
            !project.path_with_namespace.match(
              new RegExp(Config.root.filter.exclude)
            )) &&
          (Config.root.filter.excludeTags === null ||
            project.tag_list.some((tag) =>
              tag.match(new RegExp(Config.root.filter.excludeTags))
            ))
        );
      });
